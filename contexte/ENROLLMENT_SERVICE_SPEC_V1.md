...
- **Temporalité des Admissions (Admission Windows)** : Périodes strictes d'ouverture et de fermeture du recrutement, synchronisées avec le calendrier de l'année scolaire (`AcademicYear`).
...
### F17 : Pilotage du Calendrier d'Admission
- **Admission Windows** : Configuration des dates d'ouverture et de clôture des inscriptions (synchronisées avec le service Academic).
- **Auto-Lock Portail Public** : Le formulaire d'inscription en ligne devient automatiquement inaccessible hors période définie.
- **Compte à Rebours** : Affichage d'un message informatif ("Ouverture dans X jours") pour susciter l'intérêt des parents.

### F18 : Overrule & Late Entry (Porte dérobée)
- **Dérogations Administratives** : Possibilité pour le secrétariat (avec permission spéciale) de saisir un dossier même après la date de clôture officielle.
- **Traçabilité** : Chaque inscription hors-délais est marquée comme `LATE_ENROLLMENT` pour reporting à la Direction.
