...
## 9. Gestion des Classes Physiques (`/api/v1/academic/classes`)
C'est le niveau opérationnel où l'on regroupe les élèves et les profs.

### 9.1 Initialisation Automatique (Smart Create)
Lorsqu'une classe est créée (ex: "6ème A"), le système **clône automatiquement** le programme (Curriculum) du niveau vers la table des enseignements de la classe (`ClassTeachings`). 
*   **Résultat** : La classe possède immédiatement ses matières, ses coefficients et ses heures.

### 9.2 Assignation des Enseignants
L'étape suivante pour le directeur est d'assigner un prof à chaque matière de la classe.

| Méthode | Path | Action |
| :--- | :--- | :--- |
| `GET` | `/api/v1/academic/classes/{id}/teachings` | Voir les matières de la classe et les profs assignés. |
| `PATCH` | `/api/v1/academic/classes/{id}/teachings/{tId}/teacher` | Assigner un prof (`?teacherId=uuid`). |

---

## 10. Tableau des Champs Techniques (Résumé)
