  ...
  confirmPayment() {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        ...
        message: 'Confirmez-vous que le paiement des frais a été vérifié hors-système (espèces, virement) pour ce dossier ?',
    ...
    dialogRef.afterClosed().pipe(
      switchMap(confirmed => {
        if (confirmed) {
          this.isActionLoading.set(true);
          return this.enrollmentAdminService.confirmPayment(app.id);
        }
        return of(null);
      }),
      finalize(() => this.isActionLoading.set(false))
