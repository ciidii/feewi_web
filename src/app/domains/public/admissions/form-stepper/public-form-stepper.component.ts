...
export class PublicFormStepperComponent implements OnInit {
  ...
  private async saveGuardian(): Promise<boolean> {
    ...
    const bundle = await this.enrollment.createBundle({ family: payload }).toPromise();
  ...
  onDocUpload(data: { code: string; file: File }) {
    ...
    this.enrollment.getDocumentUploadTicket(admissionId, data.code, data.file.name, data.file.type, this.accessCode()).pipe(
      switchMap(ticket =>
  ...
  async saveDraftAndExit() {
    ...
    try {
      ...
      this.router.navigate(['/admissions/tracker'], {
